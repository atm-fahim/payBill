...
class UpcomingController extends BaseController
{
     ...
     * @return void
     */
    public function __construct(UpcomingRepo $upcRepo)
    {
    ...
    public function index()
    {
        ...
        $upcomingInfo                = $this->upcRepo->withoutDeletingData();
    ...
    {
        $data                           = array();
        $validation                     = $this->upcRepo->checkRequestValidity($request);
        ...
        try {
           ...
           (!empty($id)) ? $this->upcRepo->update($id, $data) : $this->upcRepo->save($data);
    ...
    {
        $upcomingInfo = $this->upcRepo->findById($id);
        echo json_encode($upcomingInfo);
    }

    public function active($id)
    {
        $this->upcRepo->update($id, ['status' => 1]);
        return redirect('upcoming-project');
    }

    public function inActive($id)
    {
        $this->upcRepo->update($id, ['status' => 0]);
        return redirect('upcoming-project');
    }

    public function delete($id)
    {
        $this->upcRepo->update($id, ['status' => 9]);
