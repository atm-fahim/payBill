...
@section('content')
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @elseif($user_access->is_approved==1)
                                    <button>
                                        <a href="{{route('payment-method-active',$value->id)}}">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">{{ __('Payment Method') }}</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form method="POST" action="{{ route('payment-method-save-update') }}"
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input id="payment_method" type="text"
                                               class="form-control @error('payment-method') is-invalid @enderror"
                                               name="payment_method" value="{{ old('payment_method') }}" required
                                               autocomplete="payment_method" autofocus>
    ...
    <script>
        function edit(id) {
            $.ajax({
                url: "<?= URL::to('/payment-method-edit'); ?>/" + id,
                type: 'get',
                data:{},
                dataType: 'JSON',
                success: function (response) {
                    ...
                    $(".id").val(response.id);
                    $("#payment_method").val(response.method_name);
                    if (response.id) {
                        $('.submit').text("Update");
