...
@section('content')
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @elseif(isset($user_access->is_approved) && ($user_access->is_approved) && $user_access->is_approved==1)
                                    <button>
                                        <a href="{{route('branch-active',$value->id)}}">
                ...
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">{{ __('Branch') }}</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form id="branchForm" method="POST" action="{{ route('branch-save-update') }}"
                                  enctype="multipart/form-data">
                                @csrf
                                <div class="row mb-3">
                                    <label for="branch" class="col-md-2 col-form-label text-md-end">{{ __('Branch') }}</label>
                                    <div class="col-md-8">
                                ...
                                <div class="row mb-3">
                                    <label for="address"
                                           class="col-md-2 col-form-label text-md-end">{{ __('About Branch') }}</label>
                                    <div class="col-md-8">
    ...
    <script>
        function edit(id) {
            // Reset the form
            $('#branchForm')[0].reset(); // Replace 'yourFormId' with your actual form ID
            ...
            $('#exampleModalLabel').text("Add Branch"); // Reset modal title

            // Now fetch the data and populate the form
            $.ajax({
                url: "<?= URL::to('/branch-edit'); ?>/" + id,
                type: 'get',
                dataType: 'JSON',
